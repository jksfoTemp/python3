...
def extract_data_from_html(html_content, outToTerm):
    try:
        ...
        job_title = (
            soup.find("h3", class_="itemTitle").get_text(strip=True)
            if soup.find("h3", class_="itemTitle")
            else "N/A"
        )
...
def main(filepath, filename, write_to_stdout=False):
    ...
    try:
        ...
        if (
            time_string
        ):  # Check if time_string is not None (meaning print_current_time was successful)
